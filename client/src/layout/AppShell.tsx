    ...
    <main className="min-h-screen bg-page text-ink">
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-4 py-5 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-4 border-b border-zinc-200 pb-5 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div
            className="inline-flex w-fit items-center gap-2 rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-700 shadow-sm"
            role="status"
          >
            <span
              className={`h-2.5 w-2.5 rounded-full ${apiStatusColor[apiStatus]}`}
              aria-hidden="true"
            />
            {apiLabel}
          </div>
